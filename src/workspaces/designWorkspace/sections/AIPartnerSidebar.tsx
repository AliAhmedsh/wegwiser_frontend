...
export default function AIPartnerSidebar({ onOptionClick }: Props) {
  const [input, setInput] = useState('');
  const { aiMessages } = useDesignWorkspaceStore();
  ...
  return (
    <div className="w-72 h-full flex flex-col rounded-xl p-2">
      <div
        className="bg-[#EAEDF2] h-full border-white rounded-xl text-sm text-black flex flex-col"
        style={{
          boxShadow: '2px 2px 2px 0px #A7B1C499, -2px -2px 2px 0px #FFFFFF',
        }}
      >
        <div className="p-4 border-b border-[#E8E8E8] font-semibold text-sm">
          AI Partner
        </div>
            ...
            <div key={msg.id}>
              {msg.kind === 'text' && msg.specialId === 'ai-question' ? (
                <div className="relative bg-white rounded-xl p-3 self-end max-w-[90%]">
                  <p className="relative z-20 text-xs font-normal">
                    {msg.text}
                  </p>
                ...
                <div
                  className="font-normal text-xs max-w-[95%]"
                  dangerouslySetInnerHTML={{ __html: msg.text }}
                />
              ) : msg.kind === 'text' ? (
                <div className="relative bg-white rounded-xl p-3 self-end max-w-[90%] shadow mt-2">
                  <div className="relative z-20 text-xs font-normal">
                    {formatAiResponseToJSX(msg.text)}
                  </div>
                  <div className="absolute z-10 -bottom-2 -right-2 w-6 h-6 rounded-full bg-white"></div>
                </div>
              ) : null}
        ...
        <form
          className="flex flex-col items-end gap-3 mt-auto p-2"
          onSubmit={handleSend}
        >
          <textarea
            rows={4}
